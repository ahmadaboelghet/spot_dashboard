...
async function test() {
    try {
        ...
        const doc = await db.collection('centers').doc('+201001133246').get();
        if(doc.exists) console.log('Center Data:', doc.data());
        else console.log('Center does not exist');
        
        const user = await admin.auth().getUserByPhoneNumber('+201001133246');
        console.log('Auth User:', user);
        
        // Also check getUserByUid
        const user2 = await admin.auth().getUser('+201001133246');
